...
if False:
    ...
    ii = 1557
    plt.figure(135)
    for n in cells[ii].nodes:
        for oc in n.cells:
            visPoly(points[oc.verts],lc=mcc[8])
            visPoint(oc.centre,lc=mcc[8],ms=10)

    for oc in cells[ii].uring:
        visPoly(points[oc.verts],lc='k')
    ...
    ii = 1661
    plt.figure(136)
    visPoly(points[cells[ii].verts],lc='k')
    plt.show(block=False)

    aa = np.array([0.2235,0.4103])   # 1830
    aa = np.array([0.1839,0.4582])   # 1661
    minDist = 100000
    mini = -1
    for i,c in enumerate(cells):
        l = np.linalg.norm(c.centre - aa)
        if l < minDist:
            minDist = l
            mini = i
...
##### UNSTRUCTURED FINITE DIFFERENCE STUFF #####
failed = []
for c in cells:
    # node ring, including this cell
    c.ua = []
    c.nring = []
    c.uring = []
    nring = []
    ...
    for n in c.nodes:
        for oc in n.cells:
            if oc.idx not in nring:
                nring.append(oc.idx)
    for i in nring:
        c.nring.append(cells[i])
    # check if we have enough
    if len(c.nring) < 6:
        print('unstructured finite difference needs ring size of >= 6 - i=',c.idx)
        continue
    # now calculate closest six
    dists = []
    for oc in c.nring:
        dists.append(np.linalg.norm(c.centre - oc.centre))
    ind = np.argsort(np.array(dists))
    # now down select
    if True:
        # closest 6
        for i in ind[:6]:
            c.uring.append(c.nring[i])
    else:   # getting singular matrices...
        # take this cell, face neighbours and then next closest up to 6
        c.uring.append(c)
        for f in c.faces:
            if f.isBoundary:
                continue
            if f.cells[0].idx != c.idx:
                c.uring.append(f.cells[0])
            else:
                c.uring.append(f.cells[1])
        i = 0
        while len(c.uring) < 6:
            usedIds = [cc.idx for cc in c.uring]
            if c.nring[ind[i]].idx not in usedIds:
                c.uring.append(c.nring[ind[i]])
            i += 1
            print(c.idx,i,len(c.uring),[cc.idx for cc in c.uring])
    #
    c.uA = np.zeros((6,len(c.uring)))
    c.uA[0,:] = 1.0
    for i,oc in enumerate(c.uring):
        c.uA[ 1,i] = oc.centre[0] - c.centre[0] # dx
        c.uA[ 2,i] = oc.centre[1] - c.centre[1] # dy
        c.uA[ 3,i] = 0.5 * c.uA[1,i]*c.uA[1,i]  # dxdx
        c.uA[ 4,i] = 0.5 * c.uA[2,i]*c.uA[2,i]  # dydy
        c.uA[ 5,i] =       c.uA[1,i]*c.uA[2,i]  # dxdy
    #
    # now solve coeffs for each gradient term separately
    # if np.linalg.cond(c.uA) < 1/sys.float_info.epsilon:
        # print(c.uA)
        # failed.append(c.idx)
        # continue
    cuAinv = np.linalg.inv(c.uA)
    for i in range(2):
        ub = np.zeros((6,1))
        ub[i+1,0] = 1
        c.ua.append( np.dot( cuAinv, ub ) )

ufd_grads = np.zeros((len(cells),2))
for c in cells:
    for i,oc in enumerate(c.uring):
        ufd_grads[c.idx,0] += c.ua[0][i] * fields[oc.idx,2]
        ufd_grads[c.idx,1] += c.ua[1][i] * fields[oc.idx,2]

# --- for 3d ---
# for i,oc in enumerate(c.nring):
#   c.uA[ 1,i] = oc.centre[0] - c.centre[0] # dx
#   c.uA[ 2,i] = oc.centre[1] - c.centre[1] # dy
#   c.uA[ 3,i] = oc.centre[2] - c.centre[2] # dz
#   c.uA[ 4,i] = 0.5 * c.uA[1,i]*c.uA[1,i]  # dxdx
#   c.uA[ 5,i] = 0.5 * c.uA[2,i]*c.uA[2,i]  # dydy
#   c.uA[ 6,i] = 0.5 * c.uA[3,i]*c.uA[3,i]  # dzdz
#   c.uA[ 7,i] =       c.uA[1,i]*c.uA[2,i]  # dxdy
#   c.uA[ 8,i] =       c.uA[1,i]*c.uA[3,i]  # dxdz
#   c.uA[ 9,i] =       c.uA[2,i]*c.uA[3,i]  # dydz
...
errGradF_UFD = np.zeros((len(cells),2))
for c in cells:
    errGradF_UFD[c.idx,0] = ufd_grads[c.idx,0] - dfdx(*c.centre)
    errGradF_UFD[c.idx,1] = ufd_grads[c.idx,1] - dfdy(*c.centre)

errL2dfdx_UFD = (np.sum(errGradF_UFD[:,0]**2.) / nElems)**0.5
errL2dfdy_UFD = (np.sum(errGradF_UFD[:,1]**2.) / nElems)**0.5
...
if False:
    ...
    print('\nfaces CWLSQ: min, max, avg of dfdx: {:.10f}, {:.10f}, {:.10f}'.format(
            np.min(errGradF_f[:,0]),
            np.max(errGradF_f[:,0]),
            np.sum(errGradF_f[:,0]) / nElems))
    print('faces CWLSQ: min, max, avg of dfdy: {:.10f}, {:.10f}, {:.10f}'.format(
            np.min(errGradF_f[:,1]),
            np.max(errGradF_f[:,1]),
            np.sum(errGradF_f[:,1]) / nElems))
    print('faces CWLSQ: L2 dfdx, dfdy: {:.10f}, {:.10f}'.format(
        errL2dfdx_f,errL2dfdy_f))
    print('\ncells CWLSQ: min, max, amin, avg of dfdx: {:.10f}, {:.10f}, {:.10f}, {:.10f}'.format(
            np.min(errGradF[:,0]),
            np.max(errGradF[:,0]),
            np.min(np.abs(errGradF[:,0])),
            np.sum(errGradF[:,0]) / nElems))
    print('cells CWLSQ: min, max, amin, avg of dfdy: {:.10f}, {:.10f}, {:.10f}, {:.10f}'.format(
            np.min(errGradF[:,1]),
            np.max(errGradF[:,1]),
            np.min(np.abs(errGradF[:,1])),
            np.sum(errGradF[:,1]) / nElems))
    print('cells CWLSQ: L2 dfdx, dfdy: {:.10f}, {:.10f}'.format(
        errL2dfdx,errL2dfdy))
    print('\ncells MGG: min, max, amin, avg of dfdx: {:.10f}, {:.10f}, {:.10f}, {:.10f}'.format(
    ...
    print('cells MGG: L2 dfdx, dfdy: {:.10f}, {:.10f}'.format(
        errL2dfdx_MGG,errL2dfdy_MGG))
    print('\ncells UFD: min, max, amin, avg of dfdx: {:.10f}, {:.10f}, {:.10f}, {:.10f}'.format(
            np.min(errGradF_UFD[:,0]),
            np.max(errGradF_UFD[:,0]),
            np.min(np.abs(errGradF_UFD[:,0])),
            np.sum(errGradF_UFD[:,0]) / nElems))
    print('cells UFD: min, max, amin, avg of dfdy: {:.10f}, {:.10f}, {:.10f}, {:.10f}'.format(
            np.min(errGradF_UFD[:,1]),
            np.max(errGradF_UFD[:,1]),
            np.min(np.abs(errGradF_UFD[:,1])),
            np.sum(errGradF_UFD[:,1]) / nElems))
    print('cells UFD: L2 dfdx, dfdy: {:.10f}, {:.10f}'.format(
        errL2dfdx_UFD,errL2dfdy_UFD))
...
if tris:
    ...
    terrGradF_UFD = np.zeros((npolyts,2))
    j = 0
    for i,c in enumerate(cells):
        for f in c.faces:
            terrGradF[j,:] = errGradF[i,:]
            terrGradF_MGG[j,:] = errGradF_MGG[i,:]
            terrGradF_UFD[j,:] = errGradF_UFD[i,:]
            j += 1
    ...
    fig = plt.figure(115,figsize=(20,20))
    if plotEdges:
        for f in faces:
            plt.plot(points[f.verts][:,0],points[f.verts][:,1], 'w-',lw=0.3)
    plt.title(r'$log$ of error of $\partial f / \partial x$, UFD',y=1.0,pad=-14)
    ax = plt.gca()
    tpc = ax.tripcolor(tpoints[:,0],tpoints[:,1],tconn[:,1:],
            facecolors=np.log(np.abs(terrGradF_UFD[:,0])),cmap=cmap,norm=norm)
    # tpc = ax.tripcolor(tpoints[:,0],tpoints[:,1],tconn[:,1:],
            # facecolors=np.abs(terrGradF[:,0]),cmap=cmap)#,norm=norm)
    fig.colorbar(tpc)
    ax.set_aspect('equal')
    ...
    fig = plt.figure(116,figsize=(20,20))
    if plotEdges:
        for f in faces:
            plt.plot(points[f.verts][:,0],points[f.verts][:,1], 'w-',lw=0.3)
    plt.title(r'$log$ of error of $\partial f / \partial y$, UFD',y=1.0,pad=-14)
    ax = plt.gca()
    # tpc = ax.tripcolor(tpoints[:,0],tpoints[:,1],tconn[:,1:],
            # facecolors=np.log(np.abs(terrGradF[:,1])),cmap=cmap,norm=norm)
    tpc = ax.tripcolor(tpoints[:,0],tpoints[:,1],tconn[:,1:],
            facecolors=np.log(np.abs(terrGradF_UFD[:,1])),cmap=cmap,norm=norm)
    # tpc = ax.tripcolor(tpoints[:,0],tpoints[:,1],tconn[:,1:],
            # facecolors=np.abs(terrGradF[:,1]),cmap=cmap)#,norm=norm)
    fig.colorbar(tpc)
    ax.set_aspect('equal')
...
urings = np.zeros((nElems,6),dtype=np.int32)
for c in cells:
    urings[c.idx,:] = [oc.idx for oc in c.uring]

if False:
    plt.figure(1)
    for i in range(6):
        plt.plot(urings[:,i],'o',label=str(i))

    plt.show(block=False)

nns = np.zeros((nElems,20),dtype=np.int32)
for c in cells:
    for i in range(len(c.nring)):
        nns[c.idx,i] = c.nring[i].idx

if False:
    plt.figure(1)
    for i in range(20):
        plt.plot(nns[:,i],'o',label=str(i))
...
fig = plt.figure(iii+131,figsize=(20,20))   # tri, s-ring
if plotEdges:
    for f in faces:
        plt.plot(points[f.verts][:,0],points[f.verts][:,1], 'w-',lw=0.3)
plt.title(r'$log$ of error of $\partial f / \partial x$, CWLSQ',y=1.0,pad=-14)
...
fig = plt.figure(iii+132,figsize=(20,20))   # tri, s-ring
if plotEdges:
    for f in faces:
        plt.plot(points[f.verts][:,0],points[f.verts][:,1], 'w-',lw=0.3)
plt.title(r'$log$ of error of $\partial f / \partial y$, CWLSQ',y=1.0,pad=-14)
...
fig = plt.figure(iii+113,figsize=(20,20))
if plotEdges:
    for f in faces:
        plt.plot(points[f.verts][:,0],points[f.verts][:,1], 'w-',lw=0.3)
plt.title(r'$log$ of error of $\partial f / \partial x$, MGG',y=1.0,pad=-14)
...
fig = plt.figure(iii+114,figsize=(20,20))
if plotEdges:
    for f in faces:
        plt.plot(points[f.verts][:,0],points[f.verts][:,1], 'w-',lw=0.3)
plt.title(r'$log$ of error of $\partial f / \partial y$, MGG',y=1.0,pad=-14)
...
fig = plt.figure(iii+115,figsize=(20,20))
if plotEdges:
    for f in faces:
        plt.plot(points[f.verts][:,0],points[f.verts][:,1], 'w-',lw=0.3)
...
fig = plt.figure(iii+116,figsize=(20,20))
if plotEdges:
    for f in faces:
        plt.plot(points[f.verts][:,0],points[f.verts][:,1], 'w-',lw=0.3)
plt.title(r'$log$ of error of $\partial f / \partial y$, UFD',y=1.0,pad=-14)
